...
The Lindblad master equation \eqref{eq:lindblad} governs the market state
$\rho_t$ and the market risk state $|\Phi_t\rangle$ in the pricing formula
\eqref{eq:pricing-master}. We analyze its structure and the classical limit
below.
...
\end{equation}
where $\hat{H}$ is the market Hamiltonian \eqref{eq:hamiltonian}, $\{L_k\}$ are
...
\subsection{The Classical Limit $\hbar_{\mathrm{econ}} \to 0$}
...
\eqref{eq:pricing-master} reduces to the classical BSM pricing formula:
\begin{equation*}
...
\end{equation*}
with $\gamma = \omega([\hat{X}, \hat{P}]_+) > 0$ (per Remark~\ref{rem:skew-sign})
...
uncertainty (Theorem~\ref{thm:uncertainty}).
