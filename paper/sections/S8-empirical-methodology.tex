...
\subsection{\texorpdfstring{$\hbar_{\mathrm{econ}}$}{hbar\_econ} Calibration via Amihud Illiquidity}
...
microstructure liquidity (Table~\ref{M-tab:qst-stats}).

\subsection{Optimization Algorithm}\label{sec:optimization}

The QST optimization problem \eqref{M-eq:mst} is solved using a projected gradient
...
\eqref{M-eq:mst} is selected by 5-fold cross-validation on the training period.
...
Table~\ref{M-tab:lead-lag} use a block length of 10 trading days on the
